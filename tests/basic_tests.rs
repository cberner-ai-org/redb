...
fn nonzero_type() {
    ...
    let write_txn = db.begin_write().unwrap();
    {
        ...
        for key in [7, -1, i32::MAX, i32::MIN, -300] {
            ...
            let value = NonZeroU64::from(key.unsigned_abs());
...
fn nonzero_type_does_not_alias_its_primitive() {
    // A `u32` table may hold zeros, which `NonZeroU32` cannot, so neither opens as the other
    let tmpfile = create_tempfile();
    let db = Database::create(tmpfile.path()).unwrap();

    let definition: TableDefinition<u32, u32> = TableDefinition::new("x");
    let nonzero_definition: TableDefinition<NonZeroU32, NonZeroU32> = TableDefinition::new("x");

    let write_txn = db.begin_write().unwrap();
    write_txn.open_table(definition).unwrap();
    write_txn.commit().unwrap();

    let read_txn = db.begin_read().unwrap();
    assert!(matches!(
        read_txn.open_table(nonzero_definition),
